package org.clauseway.functional.index;

// ABOUTME: A persistent path-trie: each node holds a value and a map of children,
// ABOUTME: addressed by key sequences; put/get/remove/sum all share structure.
...
public class ImmutableIndex<K, V> implements Index<K, V> {
	...
	public Option<Index<K, V>> get(IndexedSeq<K> seq) {
		ImmutableIndex<K, V> node = this;
		for (int i = 0; i < seq.length(); i++) {
			node = node.lookup.getOrElse(seq.get(i), null);
			if (node == null) {
				return Option.none();
			}
		}
	...
	public ImmutableIndex<K, V> sum(ImmutableIndex<K, V> other, BinaryOperator<V> valueMerger) {
		return ImmutableIndex.of(
				...
				sum(lookup, other.lookup, valueMerger).ground());
	}
