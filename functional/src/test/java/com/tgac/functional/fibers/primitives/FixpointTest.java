...
public class FixpointTest {
	...
	public void growthFeedsParkedSubscribersAbsorbedDeltasDoNot() {
		...
		assertThat(f.park("sub", v -> v.value == 0).isRight()).isTrue();
	...
	public void aParkRefusesWhenTheValueMovedPast() {
		...
		assertThat(f.park("stale", v -> v.value == 0).getLeft()).isEqualTo(MaxInt.of(1));
	...
	public void groupSealMarksEveryMemberBeforeAnyHookFires() {
		...
		assertThat(b.park("a-reader", v -> true).isRight()).isTrue();
		owners.put("b-reader", b);
		assertThat(a.park("b-reader", v -> true).isRight()).isTrue();
